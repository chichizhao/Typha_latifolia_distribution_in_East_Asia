...
## Step05: filter the variants
    ...
    vcftools --gzvcf typha_raw_filter_variants.vcf.gz --maf 0.01 --max-maf 0.99 --recode --out filtered_maf
...
## Step07: admixture analysis
    ...
    admixturePipeline.py -m popmap.txt -v typha_raw_filter_variants.vcf -k 1 -K 10 -R 8  -n 48
...
#### and we check the log, to pick up the reasonable simulation result for the demographic history inference and we plot the resultw with python script
    python script/Step_09_plot_momi.py
